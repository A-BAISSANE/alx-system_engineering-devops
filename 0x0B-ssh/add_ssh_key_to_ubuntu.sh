#!/bin/bash

# Replace with your server's IP address
SERVER_IP="54.82.196.192"

# SSH public key to add
SSH_PUBLIC_KEY="ssh-rsa "

# SSH into the server and add the SSH public key to authorized_keys
ssh ubuntu@"$SERVER_IP" "mkdir -p ~/.ssh && echo '$SSH_PUBLIC_KEY' >> ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys && sudo service ssh restart"

